...
class Discountplans extends MY_Model
{

    function __construct()
    {
        parent::__construct();
        $this->_table_name = 'discountplans';
    ...
    {
        $is_min_order_unique = $this->check_unique('min_order', $this->input->post('min_order'), ['discount_id' => $this->input->post('discount_id')]) ? "|is_unique[discountplans.min_order]" : "";
        if (!empty($this->input->post('id'))) {
            $existing = $this->get($this->input->post('id'), TRUE);
            $is_min_order_unique = (float) $existing->min_order === (float)$this->input->post('min_order') ? "" :  $is_min_order_unique;
        }
        $this->form_validation->set_rules('min_order', lang('minimum') . ' ' . lang('order'), 'required|numeric' . $is_min_order_unique, array('required' => lang('field_required_msg')));
    ...
    {
        if (!empty($entity)) {
            $entity->discount = $this->Cfgdiscounts->get($entity->discount_id, TRUE, FALSE, ['id', 'value', 'func']);
        }
        return $entity;
    }
