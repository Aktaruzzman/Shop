...
class Proddiscount extends REST_Controller
{
    ...
    public function index_post()
    {
        ...
        if (!empty($entity['bulk'])) {
            $results['status'] = $this->Prodoptions->save_by(['discount_id' => $entity['discount_id'], 'discount_by' => $entity['discount_by']], ['id >' => 0]);
            $results['status'] = $this->Proditems->save_by(['discount_id' => $entity['discount_id'], 'discount_by' => $entity['discount_by']], ['id >' => 0]);
        } else {
            if (!empty($entity['option_id'])) {
                $results['status'] = $this->Prodoptions->save(['discount_id' => $entity['discount_id'], 'discount_by' => $entity['discount_by']], $entity['option_id']);
            } else {
                if (!empty($entity['item_id']))   $results['status'] = $this->Proditems->save(['discount_id' => $entity['discount_id'], 'discount_by' => $entity['discount_by']], $entity['item_id']);
            }
        }
    ...
    private function build_entity($entity)
    {
        if (!empty($entity)) {
            if (!empty($entity->option_id)) {
                ...
                $entity->discount_id = $row->discount_id;
                $entity->discount_by = $row->discount_by;
            } else {
