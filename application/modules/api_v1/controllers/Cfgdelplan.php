...
class Cfgdelplan extends REST_Controller
{
    ...
    {
        if (!empty($config))
            $results = $this->Cfgdelplans->build_config($hub_id ? $hub_id : getallheaders()['hub_id']);
        if (!empty($id) && empty($edit)) {
            $entity = $this->Cfgdelplans->get($id, true, $deleted);
            $results['entity'] = $this->Cfgdelplans->build_entity($entity);
        } else {
            $results['list'] = $this->Cfgdelplans->get_by(['hub_id' => $hub_id ? $hub_id : getallheaders()['hub_id']], false, $deleted, $limit, $offset);
            foreach ($results['list'] as $index => $obj) {
                $results['list'][$index] = $this->Cfgdelplans->build_entity($obj);
